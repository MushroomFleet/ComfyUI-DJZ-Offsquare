...
def select_optimal_layout(image_count: int, image_dimensions: list, layout_style: str = "auto") -> dict:
    """
    Select the best layout for given images.
    """
    count = min(max(image_count, 2), 6)
    strategies = LAYOUT_STRATEGIES.get(count, LAYOUT_STRATEGIES[6])
    
    # If specific layout requested
    if layout_style != "auto":
        for strategy in strategies:
            if strategy["type"] == layout_style:
                return {
                    "type": strategy["type"],
                    "regions": apply_layout_rules(strategy["regions"], count)
                }
    
    # Auto mode: find best scoring layout
...
def get_layout_styles_for_count(count: int) -> list:
    """Get available layout style names for a given image count."""
    count = min(max(count, 2), 6)
    strategies = LAYOUT_STRATEGIES.get(count, LAYOUT_STRATEGIES[6])
    return ["auto"] + [s["type"] for s in strategies]
...
class DJZ_Offsquare:
    ...
    @classmethod
    def INPUT_TYPES(cls):
        # Get all possible layout styles across all counts
        all_styles = set(["auto"])
        for count_strategies in LAYOUT_STRATEGIES.values():
            for strategy in count_strategies:
                all_styles.add(strategy["type"])
        
        return {
            "required": {
                "images": ("IMAGE",),
                "aspect_ratio": (list(ASPECT_RATIOS.keys()), {"default": "1:1"}),
                "layout_style": (sorted(list(all_styles)), {"default": "auto"}),
                "border_radius": ("INT", {
                    "default": 0,
                    "min": 0,
                    "max": 64,
    ...
    def create_collage(self, images: torch.Tensor, aspect_ratio: str, layout_style: str, 
        ...
        Args:
            ...
            layout_style: Layout style name or "auto"
        ...
        # Select layout
        layout = select_optimal_layout(batch_size, image_dimensions, layout_style)
